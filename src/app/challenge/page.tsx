    ...
    <>
      ...
      {inProgress && (
        <Alert variant="info" title="Your challenge is already running">
          This is your reference copy. Return to the tab with your challenge workspace to
          carry on working.
        </Alert>
      )}
      ...
      <section className="border-border bg-surface shadow-(--shadow-card) space-y-4 rounded-(--radius-card) border p-6">
        ...
        <ul className="text-muted space-y-2.5 text-sm">
          <li className="flex gap-2.5">
            ...
            <span>
              You have <strong className="text-text">{formatDuration(attempt.durationMinutes)}</strong> in
              total, counted from the moment you begin. The clock runs on our servers,
              so closing your laptop or losing your connection does not pause it.
            </span>
          </li>
          <li className="flex gap-2.5">
          ...
          <li className="flex gap-2.5">
            ...
            <span>
              Your work saves as you go. You do not need to finish one challenge before
              starting another.
            </span>
          </li>
          <li className="flex gap-2.5">
            <span aria-hidden="true" className="bg-violet mt-2 size-1 shrink-0 rounded-full" />
            <span>
              Nothing is handed in until you press <strong className="text-text">Submit</strong>.
              That can only be done once, and it cannot be undone.
            </span>
          </li>
        </ul>
      </section>
      ...
      {!APPLICATION_UNDER_TEST_URL && notStarted && (
        <Alert variant="warning" title="Application link pending">
          The link to the application you will be testing in Challenge 1 will appear here
      ...
      {/* Requirement 2: shown only before the attempt starts. During the test this
          page is reference material and must not offer a way to restart anything. */}
      {notStarted && (
